...
export const formatMySQLTimeStamp = (timestamp: string | Date = new Date()): string => moment(timestamp).format(dateFormatYMD_hms);
...
export const isValidTimestamp = (timestamp) => moment(timestamp).isValid();
...
export const endOfWeek = (dateFormat) =>
	moment()
		.endOf('week')
		.format(dateFormat ? dateFormat : dateFormatYMD);
export const previousDaysFromNow = (prevDay: number, dateFormat: string = dateFormatYMD) => moment().subtract(prevDay, 'days').format(dateFormat);
...
export const fromDate = (dateFormat: any) => moment(beforeThirtyDays as moment.MomentInput, dateFormat ? dateFormat : dateFormatYMD);
