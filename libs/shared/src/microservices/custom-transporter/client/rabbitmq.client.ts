...
import { AmqpConnectionManager, ChannelWrapper, connect as connectAMQP } from 'amqp-connection-manager';
import { ConfirmChannel, ConsumeMessage } from 'amqplib';
import { EventEmitter } from 'events';
import { Observable, fromEvent, lastValueFrom, merge } from 'rxjs';
...
import { MessageOptions, PublishOptions, RMQClientOptions, RMQMessage } from '../interfaces/rmq-options.interface';
...
export class RabbitMQClient extends ClientProxy {
	...
	public mergeDisconnectEvent<T = any>(instance: any, source$: Observable<T>): Observable<T> {
	...
	protected override publish(message: ReadPacket<string | RMQMessage>, callback: (packet: WritePacket) => any): any {
		try {
	...
	public async handleMessage(packet: unknown, callback: (packet: WritePacket) => any) {
	...
	protected override dispatchEvent(packet: ReadPacket<string | RMQMessage>): Promise<any> {
		let messageOptions: MessageOptions;
		...
		return new Promise<void>((resolve, reject) =>
			this.channel.publish(exchange, packet.pattern, Buffer.from(JSON.stringify(serializedPacket)), publishOptions, (err) =>
